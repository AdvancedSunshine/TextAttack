...
def make(task = 'sst2', tokenizer_checkpoint = 'bert-base-uncased', local_path = None):  ##原版
# def make(task = 'sst2', tokenizer_checkpoint = 'bert-base-uncased', local_path = None, Train_or_Val_or_Test = 'trian'):  ##修改
    task_info = {
        'sst2':{
            'num_labels':2,
            'text_key':'text',
    ...
    if local_path is not None:
        ...
        dataset.save_to_disk(dataset_dict_path='./datasets_1/{}'.format(task))
    ...
    def preprocess_function(examples):
        if task.startswith('mnli'):
            ...
            encode = tokenizer(examples['train'].features[sentence1_key], truncation=True)
            # encode = tokenizer(data, truncation=True)
    ...
    encoded_dataset_train = dataset['train'].map(preprocess_function)
    encoded_dataset_val = dataset['validation'].map(preprocess_function)
    encoded_dataset_test = dataset['test'].map(preprocess_function)
    encoded_dataset_train.set_format("numpy")
    encoded_dataset_val.set_format("numpy")
    encoded_dataset_test.set_format("numpy")
